...
export default function Layout() {
  ...
  return (
    <div className="min-h-full flex flex-col">
      <header className="bg-teal-800 text-white shadow-sm sticky top-0 z-30">
        <div className="mx-auto max-w-7xl px-3 sm:px-6 lg:px-8">
          <div className="flex h-14 sm:h-16 items-center justify-between gap-2 sm:gap-4">
            <Link to="/" className="flex items-center gap-2 sm:gap-3 shrink-0 min-w-0">
              <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-lg bg-teal-500 flex items-center justify-center font-bold text-sm">
                EC
              </div>
              <div className="leading-tight min-w-0">
                <div className="font-semibold text-sm sm:text-base truncate">Epic Clarity</div>
                <div className="text-[10px] sm:text-xs text-teal-200">Snowflake Demo</div>
              </div>
            </Link>
            ...
            <form onSubmit={onSubmit} className="hidden md:flex flex-1 max-w-xl">
              <input
                ...
                className="flex-1 rounded-l-md border-0 px-4 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-2 focus:outline-teal-300"
              />
              <button
                type="submit"
                aria-label="Search"
                className="inline-flex items-center justify-center rounded-r-md bg-teal-600 hover:bg-teal-500 px-3 border border-teal-600"
              >
            ...
            <nav className="hidden lg:flex items-center gap-1 text-sm">
              {NAV_ITEMS.map(([to, label]) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `px-3 py-2 rounded-md transition-colors ${isActive ? 'bg-teal-700' : 'hover:bg-teal-700/60'}`
                  }
                >
                  {label}
                </NavLink>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="lg:hidden h-9 w-9 inline-flex items-center justify-center rounded-md hover:bg-teal-700/70"
              >
          ...
          {mobileOpen && (
            <div className="lg:hidden pb-3 border-t border-teal-700/40 pt-3 space-y-3">
              <form onSubmit={onSubmit} className="md:hidden flex">
                ...
                />
                <button type="submit" aria-label="Search" className="rounded-r-md bg-teal-600 hover:bg-teal-500 px-3 inline-flex items-center justify-center">
                  <SearchIcon className="h-4 w-4" />
                </button>
              </form>
              <nav className="grid grid-cols-2 gap-1 text-sm">
                {NAV_ITEMS.map(([to, label]) => (
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `px-3 py-2 rounded-md text-center ${isActive ? 'bg-teal-700' : 'bg-teal-700/30 hover:bg-teal-700/60'}`
                    }
                  >
                    {label}
                  </NavLink>
